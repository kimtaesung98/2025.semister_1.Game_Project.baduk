...
public class Pawn : Piece
{
    ...
    public override List<Tuple<int, int>> GetPossibleMoves(Board board)
    {
        ...
        if (board.IsWithinBounds(forwardRow, attackLeftCol) && board.GetPiece(forwardRow, attackLeftCol) != null && board.GetPiece(forwardRow, attackLeftCol).color != color)
        {
            possibleMoves.Add(Tuple.Create(forwardRow, attackLeftCol));
        }
        if (board.IsWithinBounds(forwardRow, attackRightCol) && board.GetPiece(forwardRow, attackRightCol) != null && board.GetPiece(forwardRow, attackRightCol).color != color)
        {
